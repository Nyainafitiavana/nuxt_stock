...
import {
  ...
  const data = ref<IExpenseType[]>([]);
  const isOpenModal = ref<boolean>(false);
  const isEdit = ref<boolean>(false);
  const isView = ref<boolean>(false);
  const formRef = ref<FormInstance>();
  const expenseTypeId = ref<string>('');
  const formState = reactive<FormExpenseType>({designation: ''});
  ...
  const handleView = (record: IExpenseType) => {
    resetForm();
  ...
  const handleEdit = (record: IExpenseType) => {
    resetForm();
    formState.designation = record.designation;

    if (record.uuid != null) {
      expenseTypeId.value = record.uuid;
    }

    handleShowModal(true, false);
  };

  const handleDelete = (record: IExpenseType) => {
    if (record.uuid != null) {
      expenseTypeId.value = record.uuid;
    }
  ...
  const insertExpenseType = async () => {
    const dataForm: FormExpenseType = formState;

    try {
      //the params userId is null here because we are in the insert method
      await insertOrUpdateExpenseType(dataForm, null, 'POST');
  ...
  const updateExpenseType = async () => {
    ...
    try {
      ...
      await insertOrUpdateExpenseType(dataForm, expenseTypeId.value, 'PATCH');
      //turn off of loading button and close modal
      loadingBtn.value = false;
      isOpenModal.value = false;
      // Show success notification
  ...
  const deleteExpenseType = async () => {
    ...
    try {
      //Call operation API in service
      await deleteExpenseTypeService(expenseTypeId.value);
      //turn off of loading button and close modal
      loadingBtn.value = false;
      isOpenModal.value = false;
      // Show success notification
